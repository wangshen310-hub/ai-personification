...
```mermaid
flowchart TD
    A["外部输入<br/>用户消息 / 时间 / 暂停 / 关系事件"] --> B["不可变事件<br/>ID / 时间 / 类型 / 数据"]
    B --> C{"事件是否已处理？"}
    C -- 是 --> C1["忽略重复事件"]
    C -- 否 --> D["推进虚拟时间"]

    subgraph K["模型外：持久人格内核"]
        D --> E["更新六种内在需求"]
        E --> F["评估情绪与心境"]
        F --> G["更新关系状态<br/>熟悉度 / 信任 / 互惠 / 边界清晰度"]
        G --> H["生成状态快照"]
    end

    H --> I["构建有界上下文"]
    I --> I1["稳定人格<br/>名字 / 特质 / 价值 / 表达风格"]
    I --> I2["需求 / 紧迫度 / 情绪"]
    I --> I3["关系状态 / 最近真实对话"]
    I1 --> J
    I2 --> J
    ...
    J["LLM / Codex / Ollama<br/>生成结构化候选"] --> L["回复 / 内部记录 / 等待 / 不行动"]
    L --> M["独立评价<br/>重复度 / 最低打扰成本"]
    M --> N["权限与边界检查"]
    N --> O{"候选是否合法？"}
    O -- 否 --> P["淘汰并记录原因"]
    O -- 是 --> Q["策略评分<br/>需求缓解 + 关系价值 + 价值一致性<br/>- 打扰 - 风险 - 重复"]
    P --> R
    Q --> R["与不行动候选比较"]
    R --> S{"最终选择"}
